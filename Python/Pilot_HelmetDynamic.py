# -*- coding: utf-8 -*-
"""
Created on Wed Sep 27 15:44:41 2023
...
for fName in entries:
    try:
        ...
        if 'No' in fName or 'Yes' in fName:
            ...
            if np.shape(dat)[0] > 200:
                ...
                sagittalRange = np.subtract(x_maxes, x_mins)
                FrontalRange = np.subtract(y_maxes, y_mins)
                TransverseRange = np.subtract(z_maxes, z_mins)
                    
                subName.extend([subTmp]* 8)
                config.extend([configTmp]*8)
                move.extend([tmpMove]*8)
    
            
            
                outcomes = pd.DataFrame({'Subject':list(subName), 'Config': list(config), 'Movement':list(move),
                                          'SagittalRange':list(sagittalRange), 'FrontalRange':list(FrontalRange), 'TransverseRange':list(TransverseRange)
                                          })
            
            
            
                outfileName = fPath + '0_CompiledDynamicHelmetData.csv'
                if save_on == 1:
