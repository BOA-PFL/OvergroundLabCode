...
fName = entries[13] #Load one file at a time
fName2 = entries[15]
# fName3 = entries[29]
...
#plt.plot(dat.FP3_COPx[landings[0]:landings[0]+50])
#plt.plot(dat.FP3_COPy[landings[0]:landings[0]+50])
...
ax1.plot(totalForce[landings[landingToPlot]:landings[landingToPlot]+50], color=color, label = 'Total Force')
ax1.plot(YtotalForce[landings[landingToPlot]:landings[landingToPlot]+50], color='black', label = 'M-L Force')
ax1.tick_params(axis='y', labelcolor=color)

ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis

ax2.set_ylabel('M-L COP')  # we already handled the x-label with ax1
ax2.plot(dat.FP3_COPy[landings[landingToPlot]:landings[landingToPlot]+50], label = 'COP Y')
#ax2.plot(dat.FP3_COPx[landings[landingToPlot]:landings[landingToPlot]+50], label = 'COP X')
h1, l1 = ax1.get_legend_handles_labels()
h2, l2 = ax2.get_legend_handles_labels()
ax1.legend(h1+h2, l1+l2, loc=2)
fig.tight_layout() 

#### Comparison plot ####fig = plt.figure()
fig = plt.figure()
ax1 = fig.add_subplot(111)
color = 'tab:red'
ax1.set_xlabel('time')
ax1.set_ylabel('TotalForce(N)', color=color)
ax1.plot(totalForce[landings[landingToPlot]:landings[landingToPlot]+50], color=color, label = 'Z Force BOA')
ax1.plot(totalForce2[landings2[landingToPlot]:landings2[landingToPlot]+50], color='black', label = 'Z force Lace')
ax1.tick_params(axis='y', labelcolor=color)

ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis

ax2.set_ylabel('M-L COP')  # we already handled the x-label with ax1
ax2.plot(dat.FP3_COPy[landings[landingToPlot]:landings[landingToPlot]+50], label = 'BOA COP Y')
ax2.plot(dat2.FP3_COPy[landings2[landingToPlot]:landings2[landingToPlot]+50], label = 'Lace COP Y')
h1, l1 = ax1.get_legend_handles_labels()
h2, l2 = ax2.get_legend_handles_labels()
ax1.legend(h1+h2, l1+l2, loc=2)
fig.tight_layout() 

BOAMax = np.max(dat.FP3_COPy[landings[landingToPlot]:landings[landingToPlot]+50])
BOAMax - dat.FP3_COPy[landings[landingToPlot]] 
    
laceMax = np.max(dat2.FP3_COPy[landings2[landingToPlot]:landings2[landingToPlot]+50])
laceMax - dat2.FP3_COPy[landings2[landingToPlot]] 