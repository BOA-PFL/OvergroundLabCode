...
fPath = 'C:\\Users\\bethany.kilpatrick\\Boa Technology Inc\\PFL - General\\Testing Segments\\AgilityPerformanceData\\2022_Tests\\CPD_TongueLocatedDial_Oct2022\\Overground\\'
...
for fName in entries:
    try:
        ...
        # if np.max(abs(dat.FP2_GRF_Z)) > np.max(abs(dat.FP1_GRF_Z)):

            
        # else:
        #     ZForce = dat.FP1_GRF_Z
        #     XForce = dat.FP1_GRF_X 
        ...
        if (tmpMove == 'Triangle') or (tmpMove == 'triangle') or (tmpMove == 'AP') or (tmpMove == 'Triangle'):
            makeVizPlot(dat, landings, takeoffs)
...
outcomes = pd.DataFrame({'Subject':list(subName), 'Config': list(config), 'Movement':list(movements),
                         'CT':list(CT), 'impulse_Z':list(impulseZ), 'impulse_X':list(impulseX), 
                          'peakGRF_Z':list(peakGRFz), 'peakGRF_X':list(peakGRFx)}) 
                          
                          # 'peakPFmom':list(peakPFmom),
